...
describe('stepMetadataToParts', () => {
  it('maps catalog parts and converts STEP units and angles', () => {
    ...
    const angleLegAxis = new Vector3(0, 1, 0).applyEuler(new Euler(...result.parts[6].rotation))
    const angleFlangeAxis = new Vector3(0, 0, 1).applyEuler(new Euler(...result.parts[6].rotation))
    expect(angleLengthAxis.x).toBeCloseTo(0)
    expect(angleLengthAxis.y).toBeCloseTo(0)
    expect(angleLengthAxis.z).toBeCloseTo(1)
    expect(angleLegAxis.x).toBeCloseTo(0)
    expect(angleLegAxis.y).toBeCloseTo(1)
    expect(angleLegAxis.z).toBeCloseTo(0)
    expect(angleFlangeAxis.x).toBeCloseTo(-1)
    expect(angleFlangeAxis.y).toBeCloseTo(0)
    expect(angleFlangeAxis.z).toBeCloseTo(0)
