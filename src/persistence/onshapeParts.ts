...
function matchPart(source: StepPartMetadata): Match | null {
  ...
  const channel = name.match(
    /\b1\s*x\s*(2|3|5)\s*x\s*1\s*x\s*(\d+)\b.*\bC-?Channel\b/i,
  );
  if (channel)
    return {
      definition: catalogPart("CCHL"),
      param1: `1x${channel[1]}`,
      param2: channel[2],
    };

  const uChannel = name.match(
    /\b2\s*x\s*2\s*x\s*2\s*x\s*(\d+)\b.*\bU-?Channel\b/i,
  );
  if (uChannel)
    return {
      definition: catalogPart("UCHL"),
      param1: "2x2",
      param2: uChannel[1],
    };

  const angle = name.match(
    /\b(1|2|3)\s*x\s*(1|2|3)\s*x\s*(\d+)\b.*\bAluminum Angle\b/i,
  );
  if (angle)
    return {
      definition: catalogPart("ANGL"),
      param1: `${angle[1]}x${angle[2]}`,
      param2: angle[3],
    };

  if (/\b(?:V5\s+)?Smart Motor\b/i.test(name)) {
    return {
      definition: catalogPart("MOTR"),
      param1: /5\.5W/i.test(name) ? "5.5W" : "11W",
    };
  }
  ...
  if (gear) {
    const type = /High Strength.*(?:V2|v2)/i.test(name)
      ? "High Strength v2"
      : /High Strength/i.test(name)
        ? "High Strength"
        : "Normal";
    return {
      definition: catalogPart("GEAR"),
      param1: type,
      param2: `${gear[1]}T`,
    };
  }
  ...
  if (/\bAnti-Static Wheel\b/i.test(name)) {
    return {
      definition: catalogPart("TWHL"),
      param1: "V2",
      param2: sizeInches(name) ?? "2.75in",
    };
  }

  if (/\bAnti-Static Omni-Directional Wheel\b/i.test(name)) {
    return {
      definition: catalogPart("OMNI"),
      param1: "V2",
      param2: sizeInches(name) ?? "2.75in",
    };
  }
  ...
  if (/\bFlex Wheel\b/i.test(name)) {
    return {
      definition: catalogPart("FWHL"),
      param1: "No Adapters",
      param2: sizeInches(name) ?? "2.00in",
    };
  }

  if (/\bNylock Nut\b/i.test(name))
    return { definition: catalogPart("NUT"), param1: "Lock" };

  if (/\bC-Channel Coupler Gusset\b/i.test(name)) {
    return {
      definition: catalogPart("GSET"),
      param1: "Coupler",
      param2: "Channel",
    };
  }

  const screw = name.match(
    /#8-32\s*x\s*(\d+-\d+\/\d+|\d+\/\d+|\d+(?:\.\d+)?)"\s+Star Drive Screw/i,
  );
  if (screw)
    return {
      definition: catalogPart("SCRW"),
      param1: catalogScrewLength(screw[1]),
    };

  const standoff = name.match(
    /\b(\d+(?:\.\d+)?|\d+\/\d+|\d+-\d+\/\d+)"\s+Long.*\bStandoff\b/i,
  );
  if (standoff) {
    return { definition: catalogPart("SNDF"), param1: `${standoff[1]}in` };
  }

  const shaft = name.match(
    /(?:^|\s)(\d+(?:\.\d+)?)"\s+(High Strength|Standard) Shaft\b/i,
  );
  if (shaft) {
    return {
  ...
  const spacer = name.match(
    /\b(1\/16|1\/8|1\/4|3\/8|1\/2)"?\s+High Strength Shaft Spacer\b/i,
  );
  if (spacer)
    return {
      definition: catalogPart("SPCR"),
  ...
  if (sprocket)
    return {
      definition: catalogPart("SPKT"),
      param1: sprocket[2] ? "High Strength" : "Normal",
      param2: `${sprocket[1]}T`,
    };

  if (/\bRotation Sensor\b/i.test(name)) {
    return {
      definition: catalogPart("SNSR"),
      param1: "Rotation",
      param2: "V5",
    };
  }
  ...
  const definition = PARTS.find((candidate) => {
    const catalogName = candidate.name
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, " ");
...
function isKnownSubcomponent(source: StepPartMetadata) {
  const name = `${source.name} ${source.productId} ${source.description ?? ""}`;
  return /\b(?:Pneumatic Cylinder Rod|Shaft Adapter|Gear Insert|VEX-HINGE-2(?:\s+RevA)?|VEX-HINGE-PIN)\b/i.test(
    name,
  );
}
...
function editorPosition(
  ...
  return converted.map((value) => Math.round(value * 1e9) / 1e9) as [
    number,
    number,
    number,
  ];
...
export function stepMetadataToParts(
  ...
  const scale = unitScale(
    "units" in metadata ? metadata.units : metadata.source.units,
  );
    ...
    const variantExists =
      ...
      definition.variants.some(
        (variant) =>
          ...
          (!variant.param2 || variant.param2 === param2),
      );
    if (!variantExists) {
      skipped.push(source);
      continue;
